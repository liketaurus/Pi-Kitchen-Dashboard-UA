(function() {
	...
	// Your city name
	var city = 'Poltava';
	var api_key = '5568563c3cae0cc7e308c291a518ccb2';
	...
	function fillForecast(day, forecast) {
		...
		if (day.length) {
			if (day === 1) {
				...
				day.html(moment(forecast.dt_txt).format('ll'));
			}
		}
	...
	function queryOpenWeatherMap() {
		...
		$.ajax({
			type: 'GET',
			url: 'https://api.openweathermap.org/data/2.5/forecast?q=' + city + '&APPID=' + api_key + '&units=' + metric + '&lang=ua',
			dataType: 'json'
		}).done(function (result) {
			// Drill down into the returned data to find the relevant weather information
			result = result.list;
